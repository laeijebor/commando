...
async function main(): Promise<void> {
  ...
  const statusObserved = (client: ClientState, paneId: string): boolean =>
  ...
  const syncRequiredSessions = (): void => {
    ...
    for (const paneId of paneTextTails.keys()) {
      if (!observedPaneIds.has(paneId)) paneTextTails.delete(paneId)
    }
  ...
  const emitAgentStatus = (paneId: string, capturedAt = Date.now()): void => {
    ...
    for (const client of clients) {
      if (!statusObserved(client, paneId)) continue
  ...
  const handleClientMessage = (client: ClientState, message: ClientMessage): void => {
    switch (message.type) {
      case 'subscribe': {
        ...
        for (const paneId of previouslyObserved) {
          if (!statusObserved(client, paneId)) client.lastStatus.delete(paneId)
        }
