...
- `emitAgentStatus` applies inferred changes to the global status registry; the
  registry broadcasts them using the existing all-session HUD behavior.
...
- `window-strip` buttons become tabs (`aria-selected`/active by selected tab, not by
