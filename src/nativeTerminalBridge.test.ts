...
describe('NativeTerminalBridge event isolation', () => {
  ...
  it('ignores malformed, stale-page, and non-monotonic shortcut events', async () => {
    ...
    receive(bridge, 3, 'host.shortcut', { key: 'w', metaKey: true })
    receive(bridge, 4, 'host.shortcut', { key: 't', metaKey: true })
    expect(shortcut.mock.calls.map(([key]) => key)).toEqual(['k', 'w', 't'])
    expect(domShortcut).toHaveBeenCalledTimes(3)
