...
Pane groups expose width, height, and corner drag handles around their outer edge. Keyboard users can focus the width or height separator and use the arrow keys; hold Shift for larger steps. Double-clicking a handle resets that dimension. Group sizes are stored locally in the browser and do not change the saved tmux workspace definition.
