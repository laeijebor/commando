...
export class DaemonClient {
  ...
  /**
   * The pane set the daemon was last told about on this socket, so a snapshot
   * that finally names a freshly created pane re-sends, and a snapshot that
   * changes nothing does not.
   */
  private sentPaneIds: string | null = null
  ...
  private syncSubscriptions(force = false): void {
    ...
    if (!force && key === this.sentPaneIds) return
    if (this.send({ type: 'subscribe', paneIds }) === null) return
    this.sentPaneIds = key
  }
  ...
  private open(isReconnect: boolean): void {
    ...
    socket.onopen = () => {
      ...
      // A new socket is subscribed to nothing and holds no lease, whatever the
      // last one was told.
      this.sentPaneIds = null
      ...
      if (this.paneHandlers.size > 0) this.syncSubscriptions(true)
    }

    socket.onmessage = (event: { data?: unknown }) => {
      const message = parseServerMessage(event.data)
      if (!message) return
      // Every message is folded into the store first (the reducer ignores the
      // pane bytes), then handed to the pane the bytes belong to, then to the
      // listeners waiting on a reply to something they sent — each exactly
      // once, and never dropped on the way.
      ...
      } else if (message.type === 'snapshot' && this.paneHandlers.size > 0) {
        // A pane created a moment ago was filtered out of the last `subscribe`
        // because no snapshot knew it yet; this is where it is asked for.
        this.syncSubscriptions()
      }
